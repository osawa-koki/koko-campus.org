...
head($obj);
?>
<p id="message">前回は「Authentication」認証について学びましたね。<br /><br />今回はもうひとつの認証、「Certification」認証(三者間認証)について学びましょう♪<br /><br />それでは、Let's laugh!!</p>
<h2>PKI</h2>
<a href="暗号?to=href_pkey">公開鍵暗号技術</a>について覚えていますか???<br /><br />特徴として、<span class="underline">受信者の暗号用の鍵は公開鍵として公開して</span>、受信者の復号用の鍵は秘密鍵として秘密に保存することがあげられますね♪<br /><br />ですけど、どうやって公開するのでしょうか?<br /><br />本人かどうか確認するために必要なもの(公開鍵)の正当性を確認する必要があるんですね、、、<br /><br />これはまるで、服を買いに行く服がない状態のよう、、、<br /><br />ということで、公開鍵の公開は<span class="underline">信頼できる第三者</span>の存在が必要不可欠ですね。<br /><br />この公開鍵を公開する信頼できる第三者のことを<span class="underline">認証局(CA)</span>といい、認証局は公開鍵の正当性を証明するために<span class="underline">ディジタル証明書(公開鍵証明書)</span>を発行します。<br /><br />この認証局を通して公開鍵を適切に公開する仕組みを<span>PKI</span>(Public Key Infrastructure)と言います。
<h2>ディジタル証明書</h2>
認証局(CA)が発行する公開鍵の正当性を証明する電子証明書のことを言います。<br /><br />ITU-T勧告のX.509に定義された内容によると、ディジタル証明書は<span class="underline">「識別情報・公開鍵・有効期限・シリアル番号・CA自身のディジタル署名」</span>を含みます。<br /><br />また、ディジタル証明書は公開鍵の正当性を証明する内容も含まれることから、「公開鍵証明書」とも呼ばれます。<br /><br />以下で、PKIに関連する用語を説明しますね♪
...
<h2>PMI</h2>
次に、<span class="underline">PMI(権限管理基盤)</span>について説明します。<br />PKIが所有者の本人性を証明するのに対して、PMIでは<span class="underline">所有者の「属性(役割・権限)」</span>を証明する基盤となります。<br /><br />以下で、PMIに関連する用語を紹介しますね♪
<div class="scroll-600w">
	<table>
...
<h2>GPKI</h2>
PKIの政府(Government)バージョンで、「政府認証基盤」といいます。<br />国民と政府との間のやり取りをネットワークを経由して行う際に、「データが本人によって作成されたものか」「データが途中で改竄されていないか」を確認するための基盤のこと言います。
<p>政府公式ページは<a href="https://www.gpki.go.jp/documents/gpki.html">こちら</a>。</p>
法務省が管理するブリッジ認証局が各認証局が発行する官職認証や職責証明書などの証明書やその失効情報などを一元的に提供して、申請者が受け取った証明書の有効性を検証することを可能にします。
<p>参考サイトは<a href="https://www.jpki.go.jp/ca/ca_rules.html">こちら</a>。</p>
<?php footer(); ?>