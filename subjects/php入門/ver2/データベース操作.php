...
<img src="../pics/phpMyAdmin.png" alt="phpMyAdminの画像" />
これがデータベースを管理する画面です。<br />ではまず肝心なデータベースを作成しましょう♪<br />データベースの画面に移ってください。
<img src="../pics/phpMyAdmin-データベース.png" alt="phpMyAdminのデータベースの画面" />
こんな感じの画面に遷移すると思います。
<img src="../pics/phpMyAdmin-データベース-charset.png" alt="phpMyAdminのデータベース(文字コード指定)の画面" />
この画面ではデータベースの名前と文字コードを設定します。<br />ここでは「koko_project」に設定しました。<br />原則として半角の英数字を指定してください。<br />また「-(ハイフン)」を使用するとその後のエスケープ処理が必要となるため極力避けてください。<br />代わりに「_(アンダースコア)」を使用してください。<br /><br />文字コードでは「utf8_general_ci」ないしは「utf8_unicode_ci」を指定してください。<br /><br />両者の違いは、「utf8_general_ci」が半角と全角の区別するのに対して、「utf8_unicode_ci」は半角と全角の違いを区別しません。<br /><br />ユーザが半角と全角を混同するリスクも考えると「utf8_unicode_ci」を使用するべきだと僕は思います。<br />ユーザの入力時に全角チェックを入れればokな話ですが、、、<br /><br />では「作成」ボタンを押してテーブルの作成画面へ移りましょう♪
<img src="../pics/phpMyAdmin-テーブル作成.png" alt="phpMyAdminのテーブル作成の画面" />
...
<img src="../pics/phpMyAdmin-カラムデータ登録.png" alt="phpMyAdminのカラムデータ登録の画面" />
...
<p>4行目では上記3つのデータを引数に渡してPDOというデータベースを操作するオブジェクトを作成して、$dbhという変数に代入します。</p>
<p>5行目ではPDOが格納されている$dbhにデータベースを操作する際の指示をします。<br />ここでは、「PDO::ATTR_ERRMODE」(エラーレポート/エラーが発生した際の処理について)を「PDO::ATTR_ERRMODE_EXCEPTION」(例外処理を実行)を設定します。<br />簡単に言えばデータベースをうまく操作できなかった際には、良い感じにそれをキャッチしてユーザに伝えます。</p>
<h3>sql文の実行</h3>
<code class="php">
	$sql = "INSERT INTO member (id,pw) VALUES (?,?)"; //sqlインジェクションというハッキング対策のため入力データを組み合わせてsql文は作成しません
	$stmt = $dbh->prepare($sql);
...
<p>1行目では上で説明した通り、sql文の模型を作成します。<br />ユーザの入力データを挿入する部分はここでは「?」としておきます。</p>
<p>2行目ではPDOオブジェクトに対して上で作成したsql文の模型を引数にprepareメソッドを実行します。</p>
<p>最後に1行目の「?」で設定した部分にユーザの入力データを挿入します。<br />「?」の順番で配列を作成してそれを引数にPDOのexecuteメソッドを実行します。<br />ここで引数として渡された配列内のデータは単純なデータとして扱われるため悪意あるコードを入力されてもデータベースは不正に操作されません。</p>
