...
<h3>ステートレス</h3>
http通信において大切なのは、<span class="u">クライアントがhttpリクエストを送信してそれに対してサーバがhttpレスポンスを返した時点でhttp通信は終了する</span>という点です。<br /><br />これをステートレス(状態がない)と言い、これでは不便な場合があります。<br /><br />例えば皆さんがあるショッピングサイトにログインするために「id」と「パスワード」をhttpリクエストに含めて送信したとします。<br />これに対してサーバは送られきた「id」と「パスワード」をデータベースからチェックしてok!ならばその人用のマイページを返します。<br /><br />http通信はここで終了です。<br /><br />ユーザがこのページから何かを買い物かごに追加しようとしてもサーバはクライアントが誰なのかを識別できないためもう一度、「id」と「パスワード」を送信する必要があります。<br /><br />購入画面に遷移する際にも、購入確認画面に遷移する際にも、実際に購入を確定する際にも同様に「id」と「パスワード」を送信する必要があります。<br /><br /><br />面倒くさいですよね、、、<br /><br />ということで、http通信そのものはセッション(状態)を管理できませんが、何らかの識別用データをhttpリクエストに挿入します。
<h2>セッション情報の格納場所</h2>
では、httpリクエストのどこにユーザ識別用のデータを挿入するかですよね♪<br /><br />具体的には以下の3つの場所があげられます。
<br />
<ul>
	<li>クエリストリング(get)</li>
...
<h3>hidden filed</h3>
通常のformタグ内にユーザを識別するための隠しデータを非表示にして差し込みます。<br />inputタのtype属性に「hidden」を設定することで実現します。
<br /><br />
<code class="html">
...
<br />
postメソッドで通信されるため一応セキュリティ上は特に問題ありません。<br />しかしながら毎回の通信に際して「id」と「パスワード」を送信するのはあまり好ましくはないです。<br /><br />「id」と「パスワード」以外の識別子を入れることでこの問題は回避できますが、何だか、、、美しくないです、、、
<h3>cookie</h3>
<span class="cancel">バターと砂糖と卵をよく混ぜて、170度に熱したオーブントースターで焦げ目が出るまで、、、</span><br /><br />httpリクエストヘッダ内のcookieという領域にユーザを識別するデータを挿入します。<br /><br />セキュリティ上安全で、かつ美しいためkokoはこのcookieを用いたセッション管理をオススメします。<br /><br />以下ではこのcookieを用いたセッション管理の方法について説明します。
<h2>セッションの開始</h2>
...
<br />
「session_start()」関数を実行するとブラウザのcookie領域にランダムな32桁の文字列が設定され、これと同じものをサーバにも設定してユーザの識別に用いられます。<br />強度を高める場合には32以上のランダムな文字列を設定する必要がありますが、この設定には「php.ini」の変更が必要となるためここでは省略します。<br /><br /><br />また、
「session_start();」の後にはセッション識別データを変更するための関数である「session_regenerate_id(true);」を入れることをオススメします。<br /><br />これはセッションハイジャック攻撃を防止するために、毎回セッションデータ(セッションid)を変更するためのものです。<br /><br />特に重要なデータを用いない場合は用いる必要がないという意見もありますが、kokoは必ず使用することをオススメします。<br />実際にセッションハイジャック攻撃の成立を妨ぐ必要性が小さくても、このサイトはセキュリティが強固です!!とアピールすることでそもそもハッキングの対象になりにくくなるからです。
<br /><br />
<code class="php">
	&lt;?php
	session_start();
...
<h2>セッション変数</h2>
これでセッションそのものは作成できました。<br />次は実際にユーザのデータを格納する特別な変数である「セッション変数」へ進みましょう♪<br /><br />ユーザ専用のデータを格納するには「$_SESSION」という連想配列を用います。<br /><br />例えばユーザがログインした際に当該ユーザのidをそのセッション変数に格納することで、どのページからでもユーザを特定できます。<br />パスワードはセッション変数に格納する必要はありません。<br />パスワードはあくまでの最初のログインチェックにのみ使用します。
<br /><br />
<code class="php">
	&lt;?php
	session_start();
...
<h3>セッション破棄</h3>
ログインに成功した場合の処理を全てifの後の「{}」で囲むのは綺麗ではないですよね、、、<br /><br />ということでログインに成功していないユーザに対してはphpプログラムを強制的に終了させます。<br /><br />このための関数には「exit()」があります。<br />また同時にユーザが他のセッション変数を持っている場合も考慮してセッション変数のリセットとをセッションそのものの破棄を行いましょう♪<br />セッション破棄は以下の3つの処理を行います。
<br />
<ol>
	<li>セッション変数のリセット</li>
	<li>セッションの破棄</li>
...
<br />kokoはアンケート機能においてはajax通信でそのセッション変数を使用しているためjavascriptからcookieデータを取得しますが、一般的にphpから取得するには「$_SESSION」同様に「$_COOKIE」で取得します。
<br />
<code class="php">
