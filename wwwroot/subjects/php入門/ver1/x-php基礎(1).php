...
<div id="profile-card">
	...
	<table id="profile-table" border="1" class="x">
		<tbody>
			<tr>
				<td>名前</td>
				<td><?php echo $name; ?></td>
...
<br />
まずはこんな処理ができるようになりましょう♪<br />では、まずはphpの基礎を学びましょう♪
<h2>phpプログラムの書き方</h2>
...
また、「&lt;?php」と「?&gt;」で囲まれていない部分は全てhtml文書として出力されます。
<br /><br />
<code class="php">
	&lt;?php
	session_start();
...
<h3>コメントアウト</h3>
phpの中にメモ書きを書くこともできます。<br />メモ書きが一行ならば「//(スラッシュ２個)」より後がコメントアウトとなり、複数行になる場合は「/*」と「*/」で囲みます。<br />具体的には以下のように書きます。
<br />
<code class="php">
	&lt;?php
...
<h3>文の終了</h3>
phpではひとつの命令の終了を表すのに「;(セミコロン)」が用いられます。<br />最初のうちはセミコロンのつけ忘れによるエラーが多いと思いますので、特に注意して下さい。
<br />
<code class="php">
	session_start() //&larr;セミコロンがないためエラーとなる
	session_start(); //&larr;これでok!です
</code>
<h2>変数と代入</h2>
変数にはデータを一時的に保存することができます。先ほどの自己紹介カードの例では「<?php echo $name; ?>」というデータを$nameという変数に保存して、最後にこれをhtmlに出力しています。<br /><br />また、変数に何らかのデータを入れることを代入と言います。<br /><br />具体的には以下のコードが変数と代入の例です。
<code class="php">
	$number = 1; //$numberという変数に「1」を代入
</code>
ここでは「$number」という変数に「1」という数値を代入しています。
<p>phpで変数を使用するには変数の最初に「$」を付ける必要があります。また変数は半角英数字と「_(アンダースコア)」のみ使用可能で、変数は数字から始めることはできません。</p>
<p>変数にデータを挿入するには「=」を用います。<br />「=」はプログラミングでは「同じ」を意味するのではなく、右のデータを左の変数に代入することを意味します。<br /><br />因みに、後ほど説明しますがプログラミングで「同じ」を用いるためには「==」もしくは「===」を用います。</p>
<h2>データの型</h2>
...
<h4>演算</h4>
整数は当然、計算(演算)が可能です。<br />演算は以下のように行います。
<br /><br />
<table class="exp">
	<tbody>
		<tr>
...
<h3>浮動小数点数型</h3>
コンピュータの事情(2進数を採用していることによる問題)により整数型と浮動小数点型を同様の型として扱えませんが、phpは自動型変換機能があるので気にしなくてokです。<br /><br />ちなみにコンピュータで「0.1 &times; 3」を計算させると「0.3000.....1」と「0.3」よりも少し大きくなってしまうんですけど、こういった問題のために整数型と浮動小数点型が分けられています。
<h3>真偽値型(論理型)</h3>
...
<h3>配列型</h3>
複数のデータを格納するデータ型です。<br />配列型には「インデックス配列型」と「連想配列型」の2つがあります。
<h4>インデックス配列</h4>
...
といった感じになります。<br /><br />バリュー(配列の各要素の値)を指定するための識別子として、インデックス番号(0, 1, 2, 3, 4, ...)が自動的に割り振られます。<br /><br /><span class="u">インデックス番号はゼロから始まる</span>ことに注意してください。<br /><br />配列内のデータは「配列[インデックス番号]」で取り出します。
<br /><br />
<code class="php">
	$ary = array(
		"データ1", "データ2", "データ3", "データ4", "データ5"
...
<h4>連想配列</h4>
配列の各要素に数字ではなく、独自に名前を付けることができます。<br /><br />連想配列では特に名前を「キー」、値を「バリュー」と呼びます。<br /><br />連想配列は「キー => バリュー」の形で作成します。
<br /><br />
<code class="php">
	$ary = array(
		"キー1" => "データ1",
...
<h2>postデータの取得</h2>
先ほどのページでフォームの作成について学びましたね♪<br />先ほどのページのフォームでデータを入力して、登録ボタンを押して次のページへ遷移させるところまで学習しました。<br /><br />次はformで送信されたデータを取得しましょう♪<br /><br />post(form)で送信されたデータは全て「$_POST」という配列に「連想配列形式」で格納されています。<br /><br />キーはformで作成した「name」の値で、バリューにはユーザが実際に入力したデータが入っています。<br /><br />例えば前のページでユーザが入力した名前を取得する際には「$_POST["name"]」で、血液型を取得するには「$_POST["blood-type"]」で、誕生日を取得するには「$_POST["birthday"]」で取得します。
<br /><br />
<code class="php">
	$name = $_POST["name"];
...
<h2>データの出力</h2>
この段階で、「$name」「$blood」「$birthday」「$hello」にはユーザが入力したデータが格納されているためこれを出力したいと思います。<br /><br />phpでhtmlに出力するには「echo」または「print」関数を使用します。<br />「echo」も「print」働きはほとんど同じですので好きな方を用いてください。<br />kokoは「echo」をオススメします。<br />理由は「echo」の方が一般的であるからです。
<br /><br />
<code class="php">
	echo "hi!"; //hi!と出力される
</code>
echoに関するphpのショートハンド(コードを簡単に書く裏技)として、「&lt;?= $data;?&gt;」があります。<br />これはshort_open_tagと呼ばれ「&lt;?php echo $data; ?&gt;」と同義で、htmlメインの文書にちょっとだけphpで何かを出力させる際に便利なのですが、kokoは推奨しません。<br />理由はxmlやその他のxmlベースの文書をhtmlに組み込む際にバグにつながる危険性があるためです。
<br /><br />
...
<br /><br />
分かりましたか???<br /><br />答えは「the sooner,the better」(早ければ早いほど良い)です。<br />分からなかった方はもう一度復習してくださいね♪
<?php footer(); ?>