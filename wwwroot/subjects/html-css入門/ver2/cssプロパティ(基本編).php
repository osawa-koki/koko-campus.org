...
-->
<h2>フォントサイズ</h2>
「font-size」プロパティで設定します。<br />ではハルジオンの「sabi」クラスが付与されたサビの部分のフォントサイズを大きくしてみましょう♪
<code class="html">
...
<div class="result part_font-size">
...
当然目的に合って使い分けるべきですが、参考までに僕の使い分け方を紹介します。<br /><br />僕は、テキストの文字サイズを指定する際には「rem」を原則として用いて、ボタンの中などの固定サイズブロックの中の文字は「px」で指定しています。<br /><br />理由はユーザビリティの観点から最近ではブラウザでルート要素のフォントサイズを変更可能になっていることが多いです。<br />「rem」を用いればこれに連動して文字サイズが変更されるためユーザにとって見やすいサイトになります。<br />しかしながら、ボタンなどの固定サイズのブロック内の文字をこれに連動させて変更するとボタン内も文字がボタンからはみ出たりしてしまうため、こういう場合には「px」で絶対値を指定します。
<h2>書体</h2>
「font-family」プロパティで設定します。<br />font-faceを使用しない限り、OSに標準で搭載されているフォントしか使用できません。<br />指定したフォントがOSにインストールされていない場合は代替のフォントで表示されます。<br />通常はこういったケースを想定して複数のフォントを指定しておくことが多いです。<br /><br />複数のフォントを設定する際には「,(カンマ)」で区切って指定します。<br />複数のフォントが指定された場合には前に書かれたフォントの優先度が高くなります。<br /><br />僕はコードの部分は「consolas」というフォントで表示し、それ以外の部分は設定せずブラウザに委ねています。
<code class="html">
...
<div class="result part_font-family">
...
<h2>フォント幅</h2>
「font-weight」プロパティで設定します。<br />「100」から「1000」までの「100」刻みで指定することも可能ですが、フォントによっては詳細に指定できない場合もあるので「bold(太い)」「normal(普通)」「細い(thin)」で指定することをオススメします。
<p class="r">thinはほとんど使わず、代わりにフォントサイズを小さくすることが多いです。</p>
<code class="html">
	&lt;p class="bold"&gt;太め&lt;/p&gt;
	&lt;p class="normal"&gt;普通&lt;/p&gt;
	&lt;p class="thin"&gt;細め&lt;/p&gt;
...
<div class="result part_font-weight">
	<p class="bold x">太め</p>
...
<h2>文字間隔</h2>
「letter-spacing」プロパティで設定します。<br />普通は「0px」か「1px」のどちらかです。
<code class="html">
...
<div class="result part_letter-spacing">
...
<h2>行間の高さ</h2>
「line-height」プロパティで設定します。<br />行間の高さを指定します。
<code class="html">
...
<div class="result border-bottom_div part_line-height">
	<p class="line-height_16px x">行間の高さは<br />16pxです。</p>
	<p class="line-height_17px x">行間の高さは<br />17pxです。</p>
	<p class="line-height_18px x">行間の高さは<br />18pxです。</p>
	<p class="line-height_20px x">行間の高さは<br />20pxです。</p>
...
<h2>文字の並び</h2>
「text-align」プロパティで設定します。<br />少し複雑なのですが、あくまでも中身がテキストの場合にのみ有効であって中身が「div」などのブロック要素の場合は適用されません。
...
<div class="result border-bottom_div part_text-align">
...
<p class="r">見やすさのため、各「p」要素の終わりに傍線を加えています。</p>
<h2>フォント色</h2>
「color」プロパティで設定します。<br />色の指定方法には主に以下の3通りあります。
<ul>
	<li>名前で指定</li>
	<li>16進数表記で指定</li>
	<li>rgba表記で指定</li>
</ul>
名前で指定する方法では、「red」「blue」「green」って感じで指定します。<br />とても簡単で分かりやすいのでこれで指定できる場合はこの方法で指定しましょう♪<br /><br />16進数で指定する方法は「#00FFFF」「#CCCC99」って感じで指定しますが複雑なのでオススメしません。<br /><br />rgbaで指定する方法ではr(red:赤)とg(green:緑)とb(blue:青)とa(alpha:透明度)の割合で色を指定します。<br />簡単かつ詳細に設定できるため、名前で指定できない場合にオススメします。<br />rgbaで簡単に色を作成するプログラムを作成したので良かったら是非訪れてください。
<a href="../../../program/イロイロ/branch" class="link-program to-イロイロ"></a>
<code class="html">
	&lt;p class="red"&gt;red&lt;/p&gt;
...
<div class="result part_background-color">
...
<h2>背景画像</h2>
「background-image」プロパティで設定します。<br />「url(画像へのパス)」で指定します。
<code class="html">
...
<div class="result part_background-image">
	<p class="x para_bg-cherry">この「p」タグでは<br />背景画像に<br />桜の画像を<br />設定しています。</p>
</div>
<h3>背景画像の繰り返し</h3>
背景画像が指定した要素より小さい場合にそれを繰り返して指定した要素全体に表示するかどうかを設定します。<br />「background-repeat」プロパティで設定します。<br />デフォルトでは指定した要素を覆うように繰り返されますが、これを繰り返さないように設定できます。<br />両方向に繰り返す場合には何も指定しないか「repeat」と指定、x方向(横方向)にのみ繰り返す場合には「repeat-x」と指定、y方向(縦方向)にのみ繰り返す場合には「repeat-y」と指定、繰り返さない場合には「no-repeat」と指定します。
<code class="html">
...
<div class="result part_background-repeat">
...
<h3>背景画像の位置</h3>
背景画像を指定した要素のどこに設定するかを指定します。<br />「background-position」で設定します。<br />「上下方向 左右方向」の順番で指定します。<br />上下方向には「top(上よせ)」「center(中央よせ)」「bottom(下よせ)」が設定可能で、左右方向には「left(左よせ)」「center(中央よせ)」「right(右よせ)」が設定可能です。<br /><br />また、上下方向・左右方向ともに「center」の場合は「center center」と書かずに「center」と一回だけ書けばok!です。
<code class="html">
...
<div class="result on-bg_norepeat part_background-position">
