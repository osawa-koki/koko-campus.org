...
<img src="../pics/position-absolute.png" alt="position(absolute)" />
コードを紹介する部分の左上に表示されている説明の部分が「position: absolute」に設定されています。
<code class="html">
	&lt;!--左上に表示される「html」という文字を親要素からの絶対位置(「position: absolute」「top: 0px」「left: 0px」)で指定--&gt;
</code>
<h3>fixed</h3>
absoluteが親要素を基準にしていたのに対して、fixedは画面(スクリーン)を基準として表示位置を指定します。<br />画面の一番上に固定して表示している「ロゴ」「html&css入門」の部分が「position: fixed」で指定されています。
<img src="../pics/position-fixed.png" alt="position(fixed)" />
<?php footer(); ?>