...
head($obj);
?>
<p id="message">ここでは、データがどのようにやり取りされるか、どのように管理されるかについて学びます。<br /><br />出題頻度はそこまで高くないので飛ばしてもok!です。<br /><br />それでは、Let's jump!!</p>
<h2>ネットワークトポロジ</h2>
トポロジ(topology)とは、「位相幾何学」の意味で、、、<br /><br />「位相幾何学」ってなんですかね、、、<br /><br />簡単に言えば、「位置・場所」とかに関する学問です。<br /><br />ネットワークトポロジとは、ネットワークのトポロジ(位置・場所)に関する考え方で、ネットワークを形成するノード(コンピュータ)がどのように結ばれているかについてのモデルです。<br /><br />具体的には以下の5種類があげられます。
<ul>
	<li>ポイントツーポイント</li>
	<li>ツリー型</li>
	<li>バス型</li>
...
<h2>ネットワーク接続</h2>
社内ネットワークを構築する際は、通常は小さな単位のLANを相互に接続して全体のネットワークを構成します。<br /><br />ここでは、LANを接続するのに用いられる機器(LAN間接続機器)を学びます。
...
<h2>スパニングツリー</h2>
ブリッジやスイッチングハブは受信したブロードキャストフレームを全てのポートに送出するため、ブロードキャストフレームが通信経路上を無限にループしてしまう可能性があります。
<p>無限にループすることをブロードキャストストームと言います。</p>
これを防ぐためのポートの一部をブロックしてフレームの無限ループを防ぐプロトコルがスパニングツリーです。
<div class="exam">
	&#9836;&#9836;&#9836; 令和2年秋試験午前&#8545;問20 &#9836;&#9836;&#9836;
...
<h2>プロキシサーバ</h2>
プロキシ(proxy)とは「代理」の意味で、プロキシサーバとは、「クライアントからの要求を受けて、クライアントの代理として他のサーバにアクセスするサーバ」を言います。<br /><br />プロキシサーバを利用する利点は主に以下の2つです。
<ul>
	<li>通信量の削減</li>
...
<h2>ロードバランサ</h2>
システムに対するリクエストを複数のサーバに分散させてサーバの負荷軽減を測る仕組みです。<br /><br />主な目的としては、<span class="underline">「拡張性」と「可用性」</span>の2つがあります。<br /><br />また、ロードバランサを実現する仕組みとして以下の2つがあげられます。
<ul>
...
<h2>伝送制御</h2>
<h3>伝送制御(広義)</h3>
広い意味での伝送制御とは、「送受信者の間でデータを確実に伝送するための制御機能」を言います。<br /><br />具体的には、以下の4つの技術から構成されます。
<ul>
	<li>伝送制御(狭義)</li>
	<li>誤り制御</li>
	<li>データリンクの確立</li>
...
<h3>伝送制御(狭義)</h3>
データリンク後に実際にデータを送信する流れの中での制御を指します。<br /><br />主に以下の4つがあります。
<div class="scroll-600w">
	<table>
...
<h2>誤り制御</h2>
データ伝送においては電気的な状態によってはビット誤りは生じる可能性があります。<br /><br />この誤りを検知するため、データに誤り制御用の情報を付加しますが、この情報は以下の2種類があります。
<div class="scroll-600w">
	<table>
...
<h2>最適なデータリンクの確立</h2>
どのように通信相手のノードとの論理的な構築を形成するかについてです。<br /><br />具体的には以下の2つの方法があります。
<ul>
	<li>コンテンション方式<br />(1対1の通信)</li>
	<li>ポーリング・セレクティング方式<br />(分岐回線)</li>
</ul>
<h2>同期方式の最適化</h2>
データの開始位置と終了位置を判別することをデータ通信の同期を言いますが、主な目的として「データを正しく送信する」ことがあげられます。<br /><br />具体的には以下の方法があります。
<ul>
...
<h2>伝送制御手順</h2>
上で説明した伝送制御技術を組み合わせて実際にデータ制御が行われますが、この手順は大きく2つに分類できます。
<ul>
	<li>ベーシック手順(SYN同期 &times; パリティ方式)</li>
	<li>HDLC(フラグ方式 &times; CRC方式)<br />&rarr; マルチリンク制御用</li>
</ul>
<div class="explanation">
	<div class="title">マルチリンク</div>
